...
public class MyOrderServiceImplTest {
    ...
    @Test
    public void submitOrderTest_givenRepositoriesAndSessionFactoryAndItemQuantityLessThanNeeded_CompletesWithoutThrowingExceptions() throws Exception {
        //Arrange
        LocalDate localDate = LocalDate.now();
        Item item1 = new Item();
        item1.setItemId(1);
        ...
        customerItem2.setCustomer(customer);
        customerItem2.setItem(item2);
        customerItem2.setQuantity(2);
        List<CustomerItem> customerItems = new ArrayList<>();
        customerItems.add(customerItem1);
        customerItems.add(customerItem2);
        List<CustomerItem>validCustomerItems=new ArrayList<>();
        validCustomerItems.add(customerItem1);
        when(sessionFactory.openSession()).thenReturn(session);
        when(session.beginTransaction()).thenReturn(transaction);
        when(customerItemRepository.getShoppingCartOfCustomer(1, session)).thenReturn(customerItems);
        doNothing().when(customerItemRepository).deleteShoppingCartOfCustomer(1, session);
        ...
        when(myOrderRepository.createNewMyOrder(any(MyOrder.class), any(Session.class))).thenReturn(myOrder);
        when(itemRepository.decrementItemQuantity(1, 3, session)).thenReturn(true);
        when(itemRepository.decrementItemQuantity(2, 2, session)).thenReturn(false);
        //when the cart given to the repository equals the valid cart continue the test
        doNothing().when(myOrderItemRepository).addCustomerItemsInOrderItems(1, validCustomerItems, session);
        //when the cart given to repository not equal the valid cart fail the test
        doThrow(new RuntimeException()).when(myOrderItemRepository).addCustomerItemsInOrderItems(anyInt(), AdditionalMatchers.not(eq(validCustomerItems)),any(Session.class));
        doNothing().when(transaction).commit();
        //Act
        MyOrder returnedOrder = myOrderService.submitOrder(1);
        //Assert
